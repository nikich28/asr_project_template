...
class BPECharTextEncoder(CharTextEncoder):
    ...
    def encode(self, text) -> Tensor:
        ...
        try:
            return Tensor(self.bpe_model.encode(text)).unsqueeze(0)
    ...
    def get_simple_alphabet(cls, args):
        ...
        return cls(alphabet=model.vocab(), bpe_model=model)

    def ctc_decode(self, inds: List[int]) -> str:
        inds = [int(x[0]) for x in groupby(inds)]
        res = self.bpe_model.decode(inds, ignore_ids=[0, 1, 2, 3])[0]
        return res